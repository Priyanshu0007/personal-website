...
export const authOptions: NextAuthOptions = {
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        const admin = await db
          .select()
          .from(allowedAdmins)
          .where(eq(allowedAdmins.email, credentials.email));
        ...
        const otpRows = await db
          ...
          .where(
            and(
              eq(otps.email, credentials.email),
              eq(otps.code, credentials.otp)
            )
          );
  ...
  secret:
    envConfig.nextAuthSecret || "fallback_secret_for_development_only_12345",
};
